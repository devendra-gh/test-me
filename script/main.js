function startApp() {
    ...
    var section6Container = new PIXI.DisplayObjectContainer();
    ...
    stage.addChild(section6Container);
    ...
    function callout($content, bezierX, bezierY, attachTo, openAt, closeAt) {
        ...
        //Get images
        var closedImage = PIXI.Sprite.fromImage("images/hotspot_closed.png");
        var openedImage = PIXI.Sprite.fromImage("images/hotspot_open.png");
        ...
        closedImage.click = closedImage.tap = function (event) {
            if (!open && animPercent <= 0) {
                _.each(calloutCloseFuncs, function (fun) {
                    fun()
                });
                open = true;
            }
        ...
        calloutCloseFuncs.push(function () {
            open = false;
        });

        calloutOpenFuncs.push(function () {
            open = true;
        });
    ...
    var heightCallout = function ($content) {
    ...
    function scrollerButtons() {
        ...
        var labelImage = PIXI.Sprite.fromImage("images/scroll/label.png");
    ...
    function clusteredImage(fileName, bezierX, bezierY, attachTo, scaleX, scaleY, secondaryImageDescriptions) {
        ...
        return function (t) {
            ...
            _.each(secondaryImageBlinks, function (fun) {
                fun(totalTime);
            });
            if (!(_.isNaN(bezierX(t)) || _.isNaN(bezierY(t)))) {
    ...
    function clusteredImagePositions(fileName, bezierX, bezierY, attachTo, scaleX, scaleY, secondaryImageDescriptions) {
        ...
        return function (t) {
            ...
            _.each(secondaryImagePositions, function (fun) {
                fun(t);
            });
            if (!(_.isNaN(bezierX(t)) || _.isNaN(bezierY(t)))) {
                container.position.x = Math.round(bezierX(t));
    ...
    //section1
    var section1_calloutsY  =   p.bezier(0, 8,
                                    p.percent(height, p.c(0), 1),
    ...
    var section1_elevatorY =    p.spline([
                                    {start: 0, end: 9, segment: p.linear(0, 9, p.c(0), height)},
                                    {start: 7, end: 9, segment: height}
                                ]);


    //section2
    var section2_elevatorY =    p.spline([
                                    {start: 0, end: 9, segment: p.linear(0, 9, p.c(0), height)},
                                    {start: 7, end: 9, segment: height}
                                ]);
    ...
    //section3
    var section3_spaceneedleX = p.minus(
                                    p.percent(p.c(0), width, 0.5),
                                    p.percent(p.c(0), imageWidth, 0.5)
                                );

    var section3_spaceneedleY = p.bezier(
                                    22, 35.4, imageHeight,
    ...
    var section3_stairsCalloutX =   p.plus(
                                        p.percent(zero, imageWidth, 0.42),
                                        section3_spaceneedleX
                                    );

    var section3_stairsCalloutY =   p.minus(
                                        section3_spaceneedleY,
                                        p.percent(zero, imageHeight, .5)
                                    );

    var section3_elevatorY =    p.spline([
                                    {   start: 23,
                                        end: 34,
                                        segment: p.bezier(23, 34, p.percent(p.c(0), height, 0.7), height, p.c(0), height)},
    ...
    //section5
    var section5_spaceneedleX = p.minus(
                                    p.c(0),
                                    p.percent(p.c(0),
                                    p.minus(imageWidth, width), 1)
                                );
    ...
    var images = [
        ...
        //Section 1
        bezierImage("images/section01/background.jpg",
            p.constant(0),
            p.bezier(0, 9, imageHeight, imageHeight, imageHeight, height),
            section1Container, p.c(1), p.c(1)),

        bezierImage("images/section01/wires.png",
            p.minus(p.percent(p.c(400), width, 0.5), p.percent(p.c(0), imageWidth, 0.5)),
            p.linear(0, 9, imageHeight, imageHeight),
            section1Container, p.c(0), p.c(1)),

        bezierImage("images/section01/elevator.png",
            p.minus(p.percent(p.c(400), width, 0.5), p.percent(p.c(0), imageWidth, 0.5)),
            section1_elevatorY,
            section1Container),

        bezierImage("images/section01/parikrama.png",
            p.minus(p.percent(p.c(280), width, 0.5), p.percent(p.c(0), imageWidth, 0.5)),
            p.linear(0, 9, imageHeight, imageHeight),
            section1Container),

        bezierImage("images/section01/wall.png",
            p.minus(p.percent(p.c(0), width, 1), p.percent(p.c(0), imageWidth, 1)),
            p.linear(0, 9, imageHeight, imageHeight),
            section1Container, p.c(1), p.c(0.5)),
        ...
        //Section 2
        bezierImage("images/section02/sky.jpg",
            p.constant(0),
            p.linear(6, 24, imageHeight, height),
            section2Container, p.c(1), p.c(1.1)),

        bezierImage("images/section02/clouds.png",
            p.linear(6, 24,
        ...
        bezierImage("images/section02/buildings.png",
            p.constant(0),
            p.linear(6, 25, imageHeight, height),
            section2Container, p.c(1), p.c(1.1)),

        /*clusteredImagePositions("images/section02/wires.png",
            ...
            [
                {
                    x: 500, y: 0, imageName: "images/section02/wires.png",
                    bezierX: p.c(0), bezierY: p.c(0)
                },
                {
                    x: 490, y: 0, imageName: "images/section02/elevator.png",
                    bezierX: p.c(0), bezierY: p.linear(7, 24, imageHeight, p.c(0))
                }
            ]),*/

        bezierImage("images/section02/wires.png",
            p.minus(p.percent(p.c(0), width, 0.5), p.percent(p.c(0), imageWidth, 0.5)),
            p.linear(6, 25, imageHeight, height),
            section2Container),

        bezierImage("images/section02/elevator.png",
            p.minus(p.percent(p.c(0), width, 0.5), p.percent(p.c(0), imageWidth, 0.5)),
            p.linear(6, 25, imageHeight, height),
            section2Container),
        ...
        bezierImage("images/section03/plane.png",
            p.spline([
        ...
        bezierImage("images/section03/wires.png",
            p.minus(p.percent(p.c(0), width, 0.5), p.percent(p.c(0), imageWidth, 0.5)),
            p.bezier(22, 35.4, imageHeight, p.percent(imageHeight, height, 0.5),
                p.percent(imageHeight, height, 0.5), height),
            section3Container),

        bezierImage("images/section03/elevator.png",
            p.minus(p.percent(p.c(0), width, 0.5), p.c(27)),
            section3_elevatorY,
            section3Container),
        ...
        //Section 4
        bezierImage("images/section04/background.jpg",
            p.linear(32, 43, p.percent(p.minus(width, imageWidth), p.c(0), 0.1), p.c(0)),
            imageHeight,
            section4Container, p.c(2), p.c(1)),

        bezierImage("images/section04/plane.png",
            p.linear(32, 43, p.percent(width, p.c(0.5), 0), width),
            p.percent(height, p.c(0), 0.8),
            section4Container),

        bezierImage("images/section04/restaurant.png",
        ...
        //Section 5
        bezierImage("images/section05/skyline.jpg",
        ...
        clusteredImage("images/section05/needle.png",
            section5_spaceneedleX,
            section5_spaceneedleY,
            section5Container, p.linear(40, 57, p.c(0.5), p.c(0.3)), p.c(1),
            [//Image cluster
                /*{x: 217, y: 448, blinkPeriod: 1.1, imageName: "images/section05/column-lights.png"},*/
                /*{x: 43, y: 356, blinkPeriod: 1.3, imageName: "images/section05/restaurant_lights.png"},*/
                {x: 18, y: 340, blinkPeriod: 3, imageName: "images/section05/observation_lights.png"},
                {x: 1100, y: 330, blinkPeriod: 2, imageName: "images/section05/parikarma.png"}
                /*{x: 195, y: 24, blinkPeriod: 2.1, imageName: "images/section05/top_light.png"},*/
            ]),
        ...
        callout(
            $(hotspotTemplate({
                ...
                pod_sharetext: "",
                pod_image: false,
                pod_content: "At 520 feet, nothing blocks you from a 360 degree panorama of the Emerald City. Mountain ranges covered in Douglas Firs. Elliott Bay’s bustling waterfront. The lights of downtown Seattle.  Mt Rainier, ever-present from afar. Head up and see what all the oohhhs and ahhhhhs are about.",
        ...
        callout(
            $(hotspotTemplate({
                ...
                pod_sharetext: "",
                pod_image: false,
                pod_content: "Space Race 2012 Winner Gregory Schneider awaits his suborbital spaceflight, awarded during the Needle’s 50th Anniversary competition.",
        ...
        bezierImage("images/transitions/trans-section1-crop.jpg",
            p.constant(0),
        ...
        bezierImage("images/transitions/trans-section2-crop.jpg",
            p.constant(0),
        ...
        bezierImage("images/transitions/trans-section3-crop.jpg",
            p.constant(0),
...
function startLoader() {
    ...
    var numAssets = assets.length + 1 + SN.fonts.families.length;
